...
let RADIOD_HOSTNAME = process.env.RADIOD_HOSTNAME || 'localhost';
...
// Load saved radiod hostname
if (fs.existsSync(RADIOD_CONFIG_FILE)) {
  RADIOD_HOSTNAME = fs.readFileSync(RADIOD_CONFIG_FILE, 'utf8').trim();
}
...
app.get('/api/radiod/discover', async (req, res) => {
  try {
    const scriptPath = path.join(__dirname, 'radiod_client.py');
    const cmd = `${PYTHON_CMD} -c "
...
"`;
    ...
    res.json({
      success: true,
      services: services,
      current: RADIOD_HOSTNAME
    });
  } catch (error) {
    console.error('❌ Failed to discover radiod services:', error.message);
    res.status(500).json({
      success: false,
      error: 'Failed to discover radiod services',
      current: RADIOD_HOSTNAME
    });
  }
});

// Get current radiod configuration
app.get('/api/radiod/current', (req, res) => {
  res.json({
    hostname: RADIOD_HOSTNAME,
    rtpDestination: SWL_RTP_DESTINATION,
    rtpPort: SWL_RTP_PORT
  });
});

// Select a radiod instance
app.post('/api/radiod/select', async (req, res) => {
  const { hostname, address } = req.body;

  // Use address (multicast) or hostname
  const newHost = address || hostname;
  ...
  console.log(`🔄 Switching radiod to: ${newHost}`);

  // Test connection to new radiod
  try {
    const cmd = `${PYTHON_CMD} -c "
from ka9q import RadiodControl
with RadiodControl('${newHost}') as control:
    print('OK')
"`;
    await execAsync(cmd, { timeout: 5000 });
  } catch (error) {
    console.error(`❌ Failed to connect to radiod at ${newHost}:`, error.message);
    return res.status(400).json({
      success: false,
      error: `Cannot connect to radiod at ${newHost}`
    });
  }

  // Update configuration
  RADIOD_HOSTNAME = newHost;

  // Save to config file
  try {
    fs.writeFileSync(RADIOD_CONFIG_FILE, newHost);
    console.log(`✅ Saved radiod hostname: ${newHost}`);
  } catch (err) {
    console.warn(`⚠️ Could not save radiod config: ${err.message}`);
  }

  // Clear channel cache since we're switching radiod
  radioProxy.channelCache.clear();
  radioProxy.activeStreams.clear();

  res.json({
    success: true,
    hostname: RADIOD_HOSTNAME,
    message: `Switched to radiod: ${newHost}`
  });
});
